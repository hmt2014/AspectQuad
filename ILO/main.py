...
def train_function(args, seed):
    ...
    if args.do_inference:
        ...
        log_file_path = f"{args.dataset}_loss_max_top-k_all.txt"

        exp_results = f"precision: {scores['precision']} recall: {scores['recall']} F1 = {scores['f1']}"

        with open(log_file_path, "a+") as f:
            string1 = "top_k: "+ str(args.top_k)+ " --------- seed: " + str(args.seed)
            f.write(string1 + "\n")
            f.write(exp_results + "\n")
...
if __name__ == '__main__':
    ...
    top_k_list = [1, 3, 5, 7, 10, 15, 20, 24]
    #top_k = 7

    for each_k in top_k_list:
        args.top_k = each_k
